...
func TestverifyToken(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setupFlags: func() {
				...
				*flagVerify = "validToken.jwt"
				*flagKey = "validPublicKey.pem"
				*flagCompact = true
			},
			expectedErr: "",
		},
		{
			...
			setupFlags: func() {
				...
				*flagVerify = "invalidToken.jwt"
				*flagKey = "validPublicKey.pem"
			},
			expectedErr: "couldn't parse token",
		},
		{
		...
		},
		{
			...
			setupFlags: func() {
				...
				*flagVerify = "unsupportedAlgToken.jwt"
				*flagKey = "validPublicKey.pem"
				*flagAlg = "HS256"
			},
			expectedErr: "couldn't parse token",
		...
		},
		{
			...
			setupFlags: func() {
				...
				*flagVerify = "validToken.jwt"
				*flagKey = "corruptPublicKey.pem"
			},
			expectedErr: "couldn't parse token",
		},
		{
			name: "Compact Output Handling",
			setupFlags: func() {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := verifyToken()
			if err != nil {
				if !errors.Is(err, jwt.ErrInvalidToken) && tt.expectedErr != "" {
			...
			os.Stdout = oldStdout
			t.Logf("Scenario '%s': passed, captured output: %s", tt.name, outputBuf.String())
...
func resetFlags() {
	...
	flagCompact = flag.Bool("compact", false, "")
	flagDebug = flag.Bool("debug", false, "")
	flagKey = flag.String("key", "", "")
	flagVerify = flag.String("verify", "", "")
}
